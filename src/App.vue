<template>
  <div class="flex justify-center items-center w-full h-screen">
    <div
      class="relative flex flex-col w-fit h-72 bg-neutral-600 rounded-2xl px-8"
    >
      <!-- Turn off the piano -->
      <div
        v-if="pianoSwitcher === false"
        class="absolute w-full h-full z-20 -ml-8"
      ></div>
      ...
      <div class="flex items-end w-full h-1/5 pb-6">
        ...
        <div class="flex flex-col justify-center items-center ml-36">
          ...
          <div
            class="z-30 flex items-center w-1 h-6 shadow shadow-black bg-slate-800 cursor-pointer"
            @click="turnOff"
          >
            <div
              v-if="pianoSwitcher"
              class="z-10 w-full h-full bg-[url('./assets/img/pushedButton.png')]"
            ></div>
            <div
              v-if="pianoSwitcher === false"
              class="w-full h-5 bg-slate-500"
            ></div>
          </div>
        </div>
        <!-- Volumeddd level -->
        <div class="flex flex-col ml-5">
          <div class="flex justify-between mb-3">
            <div
              class="circle mx-px"
              :class="
                ({ 'bg-black': gain === 0 && pianoSwitcher },
                { 'bg-slate-50': gain >= 0.2 && pianoSwitcher })
              "
            ></div>
            <div
              class="circle mx-px"
              :class="
                ({ 'bg-slate-50': gain === 0.4 && pianoSwitcher },
                { 'bg-slate-50': gain > 0.4 && pianoSwitcher })
              "
            ></div>
            <div
              class="circle mx-px"
              :class="
                ({ 'bg-slate-50': gain === 0.6 && pianoSwitcher },
                { 'bg-slate-50': gain > 0.6 && pianoSwitcher })
              "
            ></div>
            <div
              class="circle mx-px"
              :class="
                ({ 'bg-slate-50': gain === 0.8 && pianoSwitcher },
                { 'bg-slate-50': gain > 0.8 && pianoSwitcher })
              "
            ></div>
            <div
              class="circle mx-px"
              :class="{ 'bg-slate-50': gain === 1 && pianoSwitcher }"
            ></div>
          </div>
        ...
        <div
          class="flex flex-col justify-center items-center h-fit w-fit -mb-3 mr-3"
        >
          <div
            class="arrow-left flex items-center w-1 h-6 shadow shadow-black bg-slate-800 cursor-pointer"
        ...
        <div
          class="flex flex-col justify-center items-center h-fit w-fit -mb-3"
        >
          <div
            class="arrow-right flex items-center w-1 h-6 shadow shadow-black bg-slate-800 cursor-pointer"
      ...
      <div class="flex w-fit h-3/5 bg-neutral-800 px-1 pt-1 pb-5">
        <div
          v-for="(pianoButton, index) in pianoButtons"
          :key="index"
          :id="`piano-white-${index}`"
          class="flex justify-end items-start h-full cursor-pointer bg-slate-50 rounded-b-sm mb-10 mr-px"
          :class="
            ({
              'z-10 flex justify-center items-end w-3 h-4/6 select-none -translate-x-1/2 cursor-pointer text-white bg-slate-900 pb-3 -mr-3':
                pianoButton.name.split('').find((el) => el === '#') === '#',
            },
            {
              'w-6':
                pianoButton.name.split('').find((el) => el === '#') !== '#',
...
watch(pianoSwitcher, (newValue, oldValue) => {
  if (newValue === false) {
    ...
    outside.classList.remove(
      "z-20",
      "bg-[url('./assets/img/pushedButton.png')]"
    );
...
function play(pianoButton, index, gain) {
  console.log(index);
  if (
    index >= currentOctaveIndex.value[0] &&
    index <= currentOctaveIndex.value[1]
  ) {
...
function painter(pianoButton, index) {
  ...
  if (pianoButton.name.split("")[2]) {
    whiteKey.classList.add(
      "bg-[url('./assets/img/pushedBlack.png')]",
      "bg-no-repeat",
      "bg-cover"
    );
    ...
    whiteKey.classList.add(
      "bg-[url('./assets/img/pushedWhite.png')]",
      "bg-no-repeat",
      "bg-cover"
    );
  setTimeout(() => {
...
function selectOctaveDown(octave) {
  ...
  for (let i = 0; i < octaves[currentOctave.value].length; i++) {
    range.push(
      pianoButtons.findIndex(
        (el) => el.name === octaves[currentOctave.value][i]
      )
    );
  }
